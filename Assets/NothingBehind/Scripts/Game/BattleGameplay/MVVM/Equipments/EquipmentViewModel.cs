    ...
    public class EquipmentViewModel : IDisposable
    {
            ...
            ICommandProcessor commandProcessor)
        {
            ...
            foreach (var slot in equipment.Slots)
            {
                ...
                slot.EquippedItem.Subscribe(item =>
                {
                    if (item != null)
                    {
                        CreateItemViewModel(item, itemsSettings);
                        _equippedItemsMap[slot.SlotType] = item;
            ...
            if (_slotsMap.TryGetValue(slotType, out var slot))
            {
                if (slot.EquippedItem.Value != item)
                {
                    if (CanEquipItem(slotType, item))
                    {
                        ...
                        // TODO: Попробовать перенести в конструктор в виде подписки на slot.EquippedItem
        ...
        public CommandResult EquipItem(int ownerId, EquipmentSlot slot, Item item)
        {
            ...
            Debug.Log("Equip item - " + item.ItemType);
