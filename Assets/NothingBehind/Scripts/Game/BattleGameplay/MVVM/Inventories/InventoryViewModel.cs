...
namespace NothingBehind.Scripts.Game.BattleGameplay.MVVM.Inventories
{
    public class InventoryViewModel : IDisposable
    {
            ...
            if (equipmentService.EquipmentMap.TryGetValue(OwnerId, out var equipmentViewModel))
            {
                ...
                foreach (var equippedItem in EquipmentItems)
                {
                    if (equippedItem.Key is SlotType.Backpack or SlotType.ChestRig)
                    {
                        if (equippedItem.Value is GridItem gridItem)
                        {
                            ...
                            {
                                AddGridToInventory(OwnerType, OwnerId, gridItem.Grid.Value);
                            }
                        }
                    }
                }
                ...
                EquipmentItems.ObserveRemove().Subscribe(e =>
                {
                    ...
                    if (removedItem is GridItem removedGridItem)
                    {
                        ...
                        RemoveGridFromInventory(OwnerType, OwnerId, removedGridItem.Grid.Value);
                    }
                }).AddTo(_disposables);
                ...
                EquipmentItems.ObserveAdd().Subscribe(e =>
                {
                    ...
                    if (addedItem is GridItem addedGridItem)
                    {
                        ...
                        if (inventoryGrid == null)
                        {
                            AddGridToInventory(OwnerType, OwnerId, addedGridItem.Grid.Value);
                        }
                    }
                }).AddTo(_disposables);
            }
        ...
        public CommandResult AddGridToInventory(EntityType entityType, int ownerId, InventoryGrid grid)
        {
            var command = new CmdAddGridToInventory(entityType, ownerId, grid);
        ...
        public CommandResult RemoveGridFromInventory(EntityType entityType, int ownerId, InventoryGrid grid)
        {
            var command = new CmdRemoveGridFromInventory(entityType, ownerId, grid);
            var result = _commandProcessor.Process(command);
