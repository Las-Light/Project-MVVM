...
using NothingBehind.Scripts.Game.BattleGameplay.MVVM.UI.PopupA;
using NothingBehind.Scripts.Game.BattleGameplay.MVVM.UI.PopupB;
using NothingBehind.Scripts.Game.BattleGameplay.MVVM.UI.ScreenGameplay;
...
namespace NothingBehind.Scripts.Game.BattleGameplay.MVVM.UI
{
    public class GameplayUIManager : UIManager
    {
        private readonly Subject<GameplayExitParams> _exitSceneRequest;

        private readonly Subject<ExitInventoryRequestResult>
            _exitInventoryRequest; // Реквест на закрытие инвентаря передается в InventoryUIViewModel
        ...
        public GameplayUIManager(DIContainer container) : base(container)
        {
            ...
            _arsenalService = container.Resolve<ArsenalService>();
            _inputManager = container.Resolve<InputManager>();
        }

        public ScreenGameplayViewModel OpenScreenGameplay()
        {
            var viewModel = new ScreenGameplayViewModel(this, _exitSceneRequest, _playerService, _arsenalService);
        ...
        public InventoryUIViewModel OpenInventory(EntityType targetType, int targetOwnerId, int ownerId,
            Vector3 position)
        {
