...
namespace NothingBehind.Scripts.Game.Gameplay.Services.Hero
{
    public class LookHeroService
    {
        ...
        public LookHeroService(GameplayInputManager inputManager, HeroSettings heroSettings)
        {
        ...
        public void LookMouse()
        {
                ...
                _inputManager.IsAim.CurrentValue)
            {
                ...
                {
                    _mouseWorldPosition = CalculateAimMousePosition(raycastHit.point, _inputManager.LookMouse.CurrentValue);
                ...
                {
                    //_humanAnimatorController.Turn(_turnRotation, (int)_turnRotation);
                }
                else
                {
                    //_humanAnimatorController.Turn(0, 0);
                }
            }
            else
            {
                //_humanAnimatorController.Turn(0f, 0);
            }
        }
        ...
        public void LookGamepad()
        {
            if (_playerInput.currentControlScheme != AppConstants.GamepadControlScheme)
                return;
            if ((_inputManager.LookGamepad.CurrentValue != Vector2.zero && _inputManager.Move.CurrentValue == Vector2.zero) ||
                (_inputManager.IsAim.CurrentValue && _inputManager.LookGamepad.CurrentValue != Vector2.zero))
            {
                ...
                var aimDirection = new Vector3(_inputManager.LookGamepad.CurrentValue.x, 0.0f, _inputManager.LookGamepad.CurrentValue.y);
